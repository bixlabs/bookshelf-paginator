...
Paginator.prototype.prepateQuery = function() {
  ...
  if (!_.isEmpty(this.joins)) {
    baseQuery = Models.Bookshelf.knex.queryBuilder()
      .from(baseQuery.clone().as('_result')).distinct('_result.id as id');

    this.countQuery = Models.Bookshelf.knex.queryBuilder()
      .from(baseQuery.clone().as('_table'))
      .count('* as count');
  } else {
  ...
  this.dataQuery = baseQuery.clone()
    .limit(this.getLimit())
  ...
  return Promise.props({
    ...
    .then(function(result) {
      ...
      return this.modelInstance.query(function(qb) {
        ...
        qb.groupBy(this.modelInstance.tableName + '.id');
        this.whereComp(qb);
