...
Paginator.prototype.buildJoins = function() {
  ...
  this.options.filterBy.forEach(function(item) {
    if (item.indexOf('.') !== -1) {
      ...
      tableName = this.buildJoin(this.modelInstance[relation](), item);
 ...
 * @param {string} item filter passed
 */
Paginator.prototype.buildJoin = function(relation, item) {
  ...
  // Only add the join to the countQuery if it's necessary
  if (this.where[item]) {
    debugJoin('adding join to count() because exist in where', item);
    this.countQuery
