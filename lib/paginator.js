...
Paginator.prototype.prepateQuery = function() {
  ...
  this.whereComp = function(qb) {
    _.forIn(fixedWhere, function(value, key) {
      ...
      // one point is from the table
      // if we have more than one point
      // we are processing a json property of the model
      var amountDots = key.match(/\./g).length;
      var isJsonWhere = amountDots > 1;
      var match;
      var jsonColumn;
      var jsonProp;
      ...
      if (isJsonWhere) {
        if (amountDots !== 2) {
          throw new Error('Paginator does not support nested json: ' + key);
        }
        ...
        if (match.length > 1) {
          ...
          debugWhere('adding json where (', [jsonColumn, jsonProp].join(' ->> '), _this.comps[key], value, ')');
          qb.andWhereRaw(jsonColumn + ' ->> ? ' + _this.comps[key] + ' ?', [jsonProp, value]);
        }
      } else {
...
Paginator.prototype.buildJoins = function() {
  ...
  this.options.filterBy.forEach(function(item) {
    ...
    // if the current filter has a dot and it is not a json filter, then is a relation
    if (item.indexOf('.') !== -1 && _this.isRelation(_.first(item.split('.')))) {
...
/**
 * Returns if the current item is a relation
 * @param item
 * @returns {Array|boolean}
 */
Paginator.prototype.isRelation = function(item) {
  return this.options.jsonProps && this.options.jsonProps.indexOf(item) === -1;
};
