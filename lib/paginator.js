 ...
 */
Paginator.prototype.prepareQueryWhere = function() {
  var fixedWhere = {};
  var _this = this;
  _.keys(this.where).forEach(
    function(key) {
      ...
      if (_this.options.rawFilters && _this.options.rawFilters[key]) {
        fixedWhere[key] = _this.where[key];
      } else {
        fixedWhere[_this.realTableNames[key] ? _this.realTableNames[key] : key] = _this.where[key];
      }
    }
  ...
  if (!_.isEmpty(fixedWhere)) {
    this.selectQuery.where(whereComp);
  }
};
...
Paginator.prototype.prepareQuery = function() {
  var baseQuery = Models.Bookshelf.knex(this.modelInstance.tableName);

  this.buildJoins();

  this.selectQuery = this.attachJoins(baseQuery.clone());

  this.prepareQueryWhere();
  this.prepareQueryResolveOrder();

  this.prepareQueryGroup();
  this.prepareQueryCount(baseQuery.clone());
};
