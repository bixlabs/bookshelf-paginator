...
Paginator.prototype.prepateQuery = function() {
  ...
  selectQuery = baseQuery.clone()
    ...
    .select(this.modelInstance.tableName + '.' + this.modelInstance.idAttribute);

  if (!_.isEmpty(this.joins)) {
    selectQuery = Models.Bookshelf.knex(this.modelInstance.tableName)
      .join(selectQuery.clone().as('_result'), '_result.id', this.modelInstance.tableName + '.id')
      .distinct('_result.id as id');
...
Paginator.prototype.paginate = function(filter, options) {
  ...
  return Promise.props({
    ...
    .then(function(result) {
      ...
      ids = result.data.map(function(r) {
        return r.id;
      });
    ...
    .then(function(results) {
      // The results can be returned in a different order than 'select distinct ...' results
      var Collection = Models.Bookshelf.Collection.extend({ model: this.modelInstance.constructor });
      this.data = new Collection();

      ids.forEach(function(id) {
        this.data.add(results.get(id));
      }.bind(this));
